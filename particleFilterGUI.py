...
class DispParticleFilter(Tk):
    ...
    def __init__(self, motions, N=500, frameLength = 0.5, displayRealRobot = True, displayGhost = False, enableMutation = False, enableMemetic = False, enableElitism = False ):
        Tk.__init__(self)
        self.title( 'Diplay Particle Filter CS373-HW03.06')
        self.motions = motions        
        self.N = N
        self.bestElementIndex = 0

        self.frameLength = frameLength
        self.displayRealRobot = displayRealRobot        
        self.displayGhost = displayGhost

        self.enableMutation = enableMutation 
        self.enableMemetic = enableMemetic 
        self.enableElitism = enableElitism

        self.error = {'distance': world_size**2, 'bearing': 2*pi }  # init error to the maximum possible
        ...
        #Label
        textLabel = 'Current state = ' + str(self.actualState+1) + '/' + str(len(self.motions))
        self.label = Label(self, text = textLabel )
        self.label.pack(side = LEFT, padx =5, pady =5)

        self.labelErrorDistance = Label(self, text = 'Distance' ) 
        self.labelErrorDistance.pack(side = LEFT, padx =5, pady =5)

        self.labelErrorBearing = Label(self, text = 'Bearing' )               
        self.labelErrorBearing.pack(side = LEFT, padx =5, pady =5)

        self.showError()

    def formatErrorLabel(self, tag, value):
        return str(tag) + ': {0:.3f}'.format( value)

    def showError(self):

        ghost_pos = get_position(self.p)
        self.error['distance'] = sqrt ( (ghost_pos[0] - self.robot.x)**2 + (ghost_pos[1] - self.robot.y)**2  )
        self.error['bearing'] = abs(ghost_pos[2] - self.robot.orientation) % (2*pi)

        try:    
            self.labelErrorBearing.configure( text = self.formatErrorLabel( 'Delta Bearing', self.error['bearing'] ) )    
            self.labelErrorDistance.configure( text = self.formatErrorLabel( 'Delta Distance', self.error['distance'] ) )               
        except:
            pass
    ...
    '''Particle filter main loop, plus a little bit of GUI sugar :D'''
    def nextStep (self, event=None):
        self.actualState = self.actualState + 1
        if self.actualState < len(self.motions):
            #Label
            stateString = 'Current state = ' + str(self.actualState+1) + '/' + str(len(self.motions))
            self.label.configure( text = stateString )

            (self.p, self.robot, self.bestElementIndex) = particle_filter_step(self.p, self.robot, self.motions[self.actualState], self.bestElementIndex, self.enableElitism, self.enableMutation, self.enableMemetic)

            #compute error in estimation
            self.showError()
...
class DisplayParticles(Canvas):
    ...
    #            ghost = get_best_position(self.p, realRob)
...
if __name__ == "__main__":
    ...
    #Display window
    wind = DispParticleFilter ( motions, 50, displayRealRobot = True, displayGhost = True )
    wind.mainloop()